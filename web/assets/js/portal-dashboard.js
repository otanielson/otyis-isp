...
(function () {
    ...
    function switchToTab(tab) {
        ...
        function run() {
            ...
            if (metricsEl)
                metricsEl.classList.toggle('d-none', tab !== 'overview');
            runTabLoaders(tab);
        }
        var target = document.getElementById('tab-' + tab);
        if (!target && typeof window.loadTabFragment === 'function') {
            window.loadTabFragment(tab).then(run);
            return;
        }
        run();
    }
    ...
    function renderPlanStatusCell(r) {
        ...
        if (r.badge)
            parts.push('<span class="badge bg-primary-subtle text-primary border border-primary-subtle">' + esc(getPlanBadgeLabel(r.badge)) + '</span>');
        if (r.block_auto)
            parts.push('<span class="badge bg-warning text-dark">Bloqueio automatico</span>');
        return parts.join(' ');
    }
    function renderPlanRadiusCell(r) {
        var bits = [];
        if (r.quota_gb != null && r.quota_gb !== '')
            bits.push('Franquia: ' + esc(String(r.quota_gb)) + ' GB/' + esc(r.quota_period || 'monthly'));
        if (r.framed_pool)
            bits.push('Pool: ' + esc(r.framed_pool));
        if (r.vlan_id != null && r.vlan_id !== '')
            bits.push('VLAN: ' + esc(String(r.vlan_id)));
        if (!bits.length)
            return '<span class="text-muted">Padrao</span>';
    ...
    function updatePlansSummary(plans) {
        ...
        if (totalEl)
            totalEl.textContent = total + ' plano' + (total === 1 ? '' : 's');
        if (activeEl)
            activeEl.textContent = active + ' ativo' + (active === 1 ? '' : 's');
        if (blockEl)
            blockEl.textContent = blocked + ' com bloqueio';
    }
    function getFilteredPlans(plans) {
        ...
        return plans.filter(function (plan) {
            ...
            if (status === 'inactive' && active)
                return false;
            if (badge === 'none' && plan.badge)
                return false;
            if (badge && badge !== 'none' && String(plan.badge || '') !== badge)
                return false;
            if (!search)
                return true;
    ...
    function deletePlan(id) {
        if (!id || !confirm('Inativar este plano?'))
            return;
    ...
    document.addEventListener('change', function (e) {
        ...
        if (!target || !target.id)
            return;
        if (target.id === 'plansStatusFilter' || target.id === 'plansBadgeFilter')
            loadPlans();
    });
    ...
    if (btnRadiusTest) {
        btnRadiusTest.addEventListener('click', function () {
            var usernameEl = document.getElementById('radiusTestUsername');
            var passwordEl = document.getElementById('radiusTestPassword');
