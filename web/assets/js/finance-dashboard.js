...
function createFinanceModule(deps) {
    ...
    function loadFinance() {
        ...
        if (month)
            params.push('ref_month=' + encodeURIComponent(month));
        if (params.length)
            q += '?' + params.join('&');
        const infoEl = getEl('financeFilterInfo');
        if (infoEl)
            infoEl.textContent = month ? `Faturas da competência ${month}` : 'Todas as faturas';
    ...
    function openEditInvoiceModal(invId, dueDate, amount, planCode, notes, onSuccess) {
        getEl('editInvoiceId').value = String(invId);
        getEl('editInvoiceDueDate').value = dueDate ? dueDate.slice(0, 10) : '';
        getEl('editInvoiceAmount').value = amount != null ? String(Number(amount)) : '';
        getEl('editInvoicePlanCode').value = planCode ?? '';
        getEl('editInvoiceNotes').value = notes ?? '';
        ...
        if (saveBtn) {
            saveBtn.onclick = () => {
                const id = getEl('editInvoiceId')?.value;
                const due = getValue('editInvoiceDueDate');
                const amt = getEl('editInvoiceAmount')?.value;
                const plan = getValue('editInvoicePlanCode');
                const n = getEl('editInvoiceNotes')?.value ?? '';
                if (!id)
                    return;
                const payload = {};
                if (due)
    ...
    function loadCaixaMovements() {
        ...
        if (from)
            q += 'date_from=' + encodeURIComponent(from) + '&';
        if (to)
            q += 'date_to=' + encodeURIComponent(to) + '&';
        if (tipo)
        ...
        api(q)
            ...
            if (elReceita)
                elReceita.textContent = 'R$ ' + (totalReceita || 0).toFixed(2).replace('.', ',');
            if (elDespesa)
                elDespesa.textContent = 'R$ ' + (totalDespesa || 0).toFixed(2).replace('.', ',');
            if (elSaldo) {
                elSaldo.textContent = 'R$ ' + (saldo || 0).toFixed(2).replace('.', ',');
                elSaldo.style.color = saldo >= 0 ? '' : '#dc2626';
            }
            if (!rows.length) {
                out.innerHTML = '<p class="mb-0 text-muted py-4 text-center">Nenhum lançamento no período.</p>';
                return;
            }
            let html = '<div class="table-responsive"><table class="table table-sm table-hover"><thead><tr><th>Data</th><th>Tipo</th><th>Descrição</th><th>Valor</th><th>Fatura</th></tr></thead><tbody>';
            rows.forEach((r) => {
    ...
    wireFinanceMenu();
    function autoOpenDefaultPage() {
        const fn = autoOpenDefaultPage;
        if (fn.done)
            return;
        if (!document.getElementById('finance-page-titulos'))
            return;
    ...
    autoOpenDefaultPage();
    if (!autoOpenDefaultPage.done && typeof MutationObserver !== 'undefined') {
        const financeObserver = new MutationObserver(() => {
            autoOpenDefaultPage();
            if (autoOpenDefaultPage.done) {
    ...
    getEl('btnGenerateInvoices')?.addEventListener('click', () => {
        ...
        if (!confirm('Gerar faturas para ' + month + '? Serão criadas apenas para clientes que ainda não têm fatura neste mês.'))
            return;
    ...
    getEl('btnCaixaNovo')?.addEventListener('click', () => {
        ...
        const dateEl = getEl('caixaNovoDate');
        if (dateEl)
            dateEl.value = d;
        const amt = getEl('caixaNovoAmount');
        const desc = getEl('caixaNovoDescription');
        if (amt)
            amt.value = '';
        if (desc)
            desc.value = '';
    ...
    getEl('btnCaixaNovoSave')?.addEventListener('click', () => {
        ...
        const desc = getEl('caixaNovoDescription')?.value ?? null;
