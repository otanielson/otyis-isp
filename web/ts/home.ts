  ...
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        section.querySelectorAll<HTMLElement>('.home-stats__value').forEach((valueEl) => {
...
function getHomeBasePath(): string {
  const path = window.location.pathname.replace(/\/$/, '') || '/';
...
function initHomePlanos(): void {
  ...
  fetch(apiUrl)
    ...
    .then((data: { ok?: boolean; plans?: PlanFromApi[] } | null) => {
 ...
 */
function updateHeroOffer(plans: PlanFromApi[], assinarBase: string): void {
  ...
  if (speedEl) speedEl.textContent = String(featured.speed_display);
  if (unitEl) unitEl.textContent = featured.unit.toLowerCase();
  if (priceIntEl) priceIntEl.textContent = intPart;
  if (priceDecEl) priceDecEl.textContent = decPart || '00';
  if (ctaEl && 'href' in ctaEl) (ctaEl as HTMLAnchorElement).href = assinarBase + '/assinar.html?plano=' + encodeURIComponent(featured.code);
}
